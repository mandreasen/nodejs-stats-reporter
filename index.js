...
class Reporter {
	constructor(packageJson) {
		...
		// Reporter version
		this.reporterVersion = require('./package.json').version || "unknown";
	...
	reportStats() {
		if (this.report) {
			...
			for (var module in this.modules) {
				stats["module_name"] = this.modules[module].name;
				stats["module_version"] = this.modules[module].version;

				var statString = QueryString.stringify(stats);
				...
				var request = HTTPS.request({
					...
					if (this.debug) {
						console.log("-----------------------------------------------------------");
						console.log("Stats reported for " + stats.module_name + "@" + stats.module_version);
